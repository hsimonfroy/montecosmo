...
def infer_model(mesh_length, eh_approx=True, oversamp=0, s8=False):
    ...
    save_dir = save_dir / f"lpt_{mesh_length:d}_640"
    ...
    model = FieldLevelModel(**default_config | 
                            ...
                            'paint_deconv': False, # whether to deconvolve painted field
    ...
    model.block()
    params_start = jit(vmap(partial(model.kaiser_post, delta_obs=model.count2delta(truth['obs']), scale_field=5/6)))(jr.split(jr.key(45), n_chains))
