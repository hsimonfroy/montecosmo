...
# @tm.python_app
def infer_model(data_type, mesh_length, eh_approx=True, oversamp=0, s8=False, select=False, png_type=None, fourier=False):
    ...
    if data_type == 'abacus':
        ...
        save_dir = main_dir / (f"tracer_rsdflat_eh{eh_approx:d}_ovsamp{oversamp:d}_s8{s8:d}" + ("_fNL" if png_type=='fNL' else "_fNLb" if png_type=='fNL_bias' else ""))
    elif data_type == 'fpm':
    ...
    save_dir /= f"lpt_{mesh_length:d}" + (f"_sel" if select else "") + f"_fNL{fNL_true:.0f}" + ("_fourier" if fourier else "")\
        #   + "_init0"
    ...
    # Load #
    ########
    box_size = 3*(2000,) if data_type == 'abacus' else 3*(2760,)
    z_obs = 0.8 if data_type == 'abacus' else 1.
    ...
    model = FieldLevelModel(**default_config | 
                            ...
                            'selection': None, # if float, padded fraction, if str or Path, path to selection file
                            ...
                            # 'lik_type': 'gaussian_fourier' if fourier else 'gaussian_delta_power',
                            'png_type': png_type,
                            } | oversamp_config)
    ...
    truth = {
        ...
        'alpha_iso': 1.,
        'alpha_ap': 1.,
        'ngbars': 8.43318125e-4 if data_type == 'abacus' else 1e-4,
    ...
    if select:
        # Add selection, with footprint-rescaling trick because nobody can provide simulations big enough :'/
        fake_box_size = 3*(2.5*2000.,)
        fits_path = Path("/global/cfs/cdirs/desi/survey/catalogs/Y1/mocks/SecondGenMocks/AbacusSummit_v4_2/mock0/LRG_complete_NGC_1_clustering.ran.fits")
        model.add_selection(fits_path, cell_budget=mesh_length**3, padding=0., box_size=fake_box_size, save_path=load_dir / f"selection_{mesh_length}.npz")
        model.cell_length = box_size[0] / mesh_length
        model.__post_init__() # update other model attributes
    # model.render()
    print(model)
    ...
    if data_type == 'abacus':
        ...
        # obs_mesh = jnp.load(load_dir / f'cdm_paint2_interl2_deconv1_{mesh_length}.npy')

        # Abacus tracer real or redshift-space
        if select:
            obs_mesh = jnp.load(load_dir / f'tracer_rsdcut_paint2_deconv1_{mesh_length}.npy')
        else:
            ...
            # obs_mesh = jnp.load(load_dir / f'tracer_real_paint2_deconv1_{mesh_length}.npy')
            obs_mesh = jnp.load(load_dir / f'tracer_rsdflat_paint2_deconv1_{mesh_length}.npy')
    ...
    from montecosmo.script import load_model, warmup1, warmup2run, make_chains, print_mclmc_config
                        ...
                        for select in selects:
                            for png_type in png_types:
                                for fourier in fouriers:
                                    print(f"\n=== data: {data_type}, mesh_length: {mesh_length}, eh_approx: {eh_approx}, oversamp: {oversamp}, s8: {s8}, sel: {select}, png_type: {png_type}, fourier: {fourier} ===")
                                    infer_model(data_type=data_type, mesh_length=mesh_length, eh_approx=eh_approx, oversamp=oversamp, s8=s8, 
