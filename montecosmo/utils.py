...
# def _rg2cgh(mesh, part="real", amp=False, norm="backward"):
#     """
#     Return the real and imaginary parts of a complex Gaussian Hermitian tensor
#     obtained by permuting and reweighting a real Gaussian tensor (3D).
#     Handle the Hermitian symmetry, specifically at border faces, edges, and vertices.
...
# def _cgh2rg(meshk, part="real", amp=False, norm="backward"):
#     """
#     Return the "real" and "imaginary" partitions of a real Gaussian tensor (3D)
#     obtained by permuting and reweighting the real or imaginary parts of a 
#     complex Gaussian Hermitian tensor.
...
#     In particular `rg2cgh(N(0,I), norm)` is distributed as `rfftn(N(0,I), norm)`
#     """
#     real = _rg2cgh(mesh, part="real", amp=amp, norm=norm)
#     if not amp:
#         imag = _rg2cgh(mesh, part="imag", amp=amp, norm=norm)
#         return real + 1j * imag
#     else:
...
#     In particular `rg2cgh(N(0,I), norm)` is distributed as `rfftn(N(0,I), norm)`
#     """
#     real = _cgh2rg(meshk.real, part="real", amp=amp, norm=norm)
#     if not amp:
#         imag = _cgh2rg(meshk.imag, part="imag", amp=amp, norm=norm)
#     else:
#         # Give same amplitude to wavevector real and imaginary part
#         imag = _cgh2rg(meshk.real, part="imag", amp=amp, norm=norm)
#     return real + imag
...
#     id = np.zeros(kshape, dtype=int)
#     xyz = np.indices(shape, dtype=int)
#     ravel_id = partial(np.ravel_multi_index, dims=shape)
...
#         assert part == "real", "part must be either 'real' or 'imag'."
#         slix, sliy, sliz = slice(1, hx), slice(1, hy), slice(1, hz)
#     id[:,:,1:-1] = ravel_id(xyz[:,:,:,sliz])
        
#     for k in [0,hz]: # two faces
#         id[:,1:hy,k] = ravel_id(xyz[:,:,sliy,k])
#         id[1:,hy+1:,k] = ravel_id(xyz[:,1:,sliy,k][...,::-1,::-1])
#         id[0,hy+1:,k] = ravel_id(xyz[:,0,sliy,k][...,::-1]) # handle the border
#         if part == "imag":
#             weights[:,hy+1:,k] *= -1

#         for j in [0,hy]: # two edges per faces
#             id[1:hx,j,k] = ravel_id(xyz[:,slix,j,k])
#             id[hx+1:,j,k] = ravel_id(xyz[:,slix,j,k][...,::-1])
#             if part == "imag":
#                 weights[hx+1:,j,k] *= -1

#             for i in [0,hx]: # two points per edges
#                 id[i,j,k] = ravel_id(xyz[:,i,j,k])
#                 if part == "imag":
#                     weights[i,j,k] *= 0.
#                 else:
#                     weights[i,j,k] *= 2**.5
